...
function extractTrailingDomainLiteral(senderEmail) {
  ...
  if (literalStart === -1)
    return;
  let literal = senderEmail.slice(literalStart + 1);
  return MESSAGE_ID_DOMAIN_LITERAL.test(literal) ? literal : void 0;
}
...
function getMessageIdDomain(senderEmail) {
  let literal = extractTrailingDomainLiteral(senderEmail);
  if (literal !== void 0)
    return literal;
...
async function handleEmail(params, request, service, workerName, env, ctx, captureContext = {}) {
...
function hasStringTag(value, tag) {
  return typeof value == "object" && value !== null && value[Symbol.toStringTag] === tag;
}
function isHeadersLike(value) {
  return hasStringTag(value, "Headers") && typeof value.entries == "function" && typeof value.get == "function";
}
function isRequestLike(value) {
  return hasStringTag(value, "Request") && typeof value.method == "string" && isHeadersLike(value.headers);
}
function isResponseLike(value) {
  return hasStringTag(value, "Response") && typeof value.status == "number" && isHeadersLike(value.headers);
}
